		...
		for(int i=0;i<trig._v.size();i++){
			for(int j=0;j<21;j++){
				if(weights_list[i][j] != 0)	{
					...
					temp[0] += weights_list[i][j] * ( trig_vback[i][0] + inverseMatrix(last_coor[bone_list[j].v1()])[0] + joint_world_coor[bone_list[j].v1()][0] );
					temp[1] += weights_list[i][j] * ( trig_vback[i][1] + inverseMatrix(last_coor[bone_list[j].v1()])[1] + joint_world_coor[bone_list[j].v1()][1] );
					temp[2] += weights_list[i][j] * ( trig_vback[i][2] + inverseMatrix(last_coor[bone_list[j].v1()])[2] + joint_world_coor[bone_list[j].v1()][2] );
				}	
			}
			trig._v[i] = temp;
			temp -= temp;
